...
app.post('/posttrip', function(req, res) {
    ...
    if(item) {
        ...
        qry = "INSERT INTO JSON (JSON) VALUES (?)";
        var params = [carid, item.trip_date, item.odometer, item.engine_oil_life, item.tire_left_front_pressure, item.tire_right_front_pressure,
            ...
            item.tire_comment, item.glass_condition, item.glass_comment, item.body_condition, item.body_comment, item.trip_comment, item.user_id];
